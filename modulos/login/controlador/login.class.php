...
class login
{
    ...
    public function iniciarSesion($nombre_usuario,$password)
    {
        ...
        if(isset ($usuario) ){
            ...
            header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/login/vistas/iniciar_sesion.php?err='.urlencode('Usuario no registrado'));
        }
    ...
    public function registrarUsuario($nombre_usuario,$email,$password,$password_confirmar)
    {
        ...
        if($password==$password_confirmar)
        {
            ...
            if( $existe==true )
            {
                header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/login/vistas/registrar_usuario.php?err='.urlencode('El nombre de usuario ya existe'));
            }  else {
                ...
                if($resultado==true){
                    ...
                    header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/login/vistas/registrar_usuario.php?err='.urlencode('Error al crear el usuario'));
                }
            }
        }else{
            header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/login/vistas/registrar_usuario.php?err='.urlencode('No coinciden las contrase&ntilde;as'));
        }
