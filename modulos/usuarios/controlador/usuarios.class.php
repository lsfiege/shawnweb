...
if (isset($_POST["modificar-usuario"]))
{
    ...
    $controlador_usuarios->modificarUsuario($nombre_usuario,$email,$password,$password_confirmar);
        ...
        header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/usuarios/vistas/modificar_datos_perfil.php');
}   
    ...
    public function modificarUsuario($nombre_usuario,$email,$password,$password_confirmar)
    {
        if($password==$password_confirmar)
        {
            ...
            if($usuario !== null){
                ...
                header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/usuarios/vistas/modificar_datos_perfil.php?ok='.urlencode('Los datos se modificaron correctamente'));
            }else{
                //error interno al persistir el usuario
                header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/usuarios/vistas/modificar_datos_perfil.php?err='.urlencode('Error al modificar el usuario'));
            }
        }else{
            header('Location: http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/modulos/usuarios/vistas/modificar_datos_perfil.php?err='.urlencode('No coinciden las contrase&ntilde;as'));
        }
